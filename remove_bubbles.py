#!/usr/bin/env python3
"""
Speech Bubble Remover: A production-quality tool to detect and remove speech bubbles
from comic panels, preserving the directory structure.

This script processes all panels in clean_panels directory, makes speech bubbles
transparent, and saves results to panels_no_bubbles directory.
...
def process_image(input_path, output_path):
    ...
    - Makes interior bubbles transparent
    - Crops off top bubbles that span most of the panel width
    """
    try:
        # Detect text regions
        ...
        # Save the result (ensure PNG format for alpha channel)
...
def process_all_panels(input_dir, output_dir):
    """
    Process all panel images found in input_dir and its subdirectories,
    preserving the directory structure in output_dir.
    """
    # Get all PNG files in the input directory and all subdirectories
    ...
    for img_path in tqdm(image_files, desc="Processing images"):
        # Create the same directory structure in output dir
        ...
        # Process the image
        has_bubbles = process_image(img_path, out_path)
    ...
    print(f"Found and removed speech bubbles in {bubbles_found} images")
...
def main():
    ...
    # Default directories
    input_dir = "clean_panels"
    output_dir = "panels_no_bubbles"
    
    # Parse command line arguments
    parser = argparse.ArgumentParser(description="Process all panel images to remove speech bubbles")
    parser.add_argument("--input", default=input_dir, help=f"Input directory (default: {input_dir})")
    ...
    # Process all panels
    process_all_panels(args.input, args.output)
